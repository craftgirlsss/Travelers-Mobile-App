...
class TripDetailScreen extends GetView<TripDetailController> {
        ...
        body: RefreshIndicator(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                _imageDestination(size, companyProviderImageURL: trip.provider.companyLogoPath, imageURL: mainImage, location: trip.location, title: trip.title),
          ...
          children: [
                ...
                Row(
                  children: [
                    ...
                    Padding(
                      ...
                      child: Text(controller.numOfPeople.value.toString(), style: Get.textTheme.titleLarge),
                    ),
            ...
            Row(
              ...
              children: [
                ...
                Text(NumberFormat.currency(locale: 'id_ID', symbol: 'Rp', decimalDigits: 0).format(controller.calculatedPrice.value), style: Get.textTheme.titleLarge?.copyWith(color: AppTheme.primaryColor, fontWeight: FontWeight.bold),
                ),
              ],
            ),
          ],
        ...
        actions: [
          ...
          Obx(() => ElevatedButton(
            onPressed: controller.isBookingLoading.value ? null : () {
              controller.performBooking(controller.numOfPeople.value).then((result){
                Get.back();
              });
            },
            child: controller.isBookingLoading.value ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2.0)) : const Text('Book Now'),
          )),
        ],
      ),
    );
  ...
  Widget _buildBottomActionButtons(int price, BookingStatusModel? bookingStatus, {
    ...
    if (!tripIsAlreadyBooked) {
      actionWidget = _buildActionButton(text: "Book Now", onPressed: onBookPressed, isLoading: isBookingLoading, isDisabled: isBookingLoading);
        ...
        children: [
          Flexible(
            child: TextButton(
              ...
              child: Text('Cancel Book', style: TextStyle(color: Colors.red.shade700, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis),
            ),
          ),
          const SizedBox(width: 10),
          Expanded(
            child: _buildActionButton(text: "Pay Now", onPressed: onPayNowPressed, isLoading: false, isDisabled: false),
          ),
        ],
      );
    } else { // isBooked && isPaid
      actionWidget = _buildActionButton(text: "Detail Payment", onPressed: onViewDetailPressed, isLoading: false, isDisabled: false);
    }
    return Container(
          ...
          Flexible(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(currencyFormatter.format(price), style: Get.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900, color: Colors.black), overflow: TextOverflow.ellipsis),
              ],
            ),
          ),
          const SizedBox(width: 10), // Tambahkan sedikit jarak
          Flexible(flex: 3, child: actionWidget),
        ],
      ),
    );
  }
  ...
  Widget _buildActionButton({required String text, required VoidCallback onPressed,required bool isLoading, required bool isDisabled}) {
    return ElevatedButton(
      ...
      child: isLoading
        ...
        : Text(text, style: Get.textTheme.titleMedium?.copyWith(color: Colors.white, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
    );
  }
    ...
    if (controller.activeTab.value == 'Details') {
      ...
      return Column(
        ...
        children: [
          ...
          const SizedBox(height: 30),
          Text('Description', style: Get.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
          const SizedBox(height: 10),
          Text(trip.description, style: Get.textTheme.bodyMedium?.copyWith(height: 1.5, color: Colors.grey[700])),
          const SizedBox(height: 30),
          _buildGroupMembersWidget(),
          const SizedBox(height: 30),
          Text('Informasi Perjalanan', style: Get.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
          const SizedBox(height: 10),
          ...
          if (accommodation?.hasAccommodation == true) _buildAccommodationInfo(accommodation!.details),
          if (guide != null) _buildGuideInfo(guide.profilePhotoPath, guide.name, guide.specialization, guide.phoneNumber),
          if (trip.vehicle != null) _buildVehicleInfo(trip.vehicle!),
          const SizedBox(height: 30),
        ],
      );
    } else {
  ...
  Widget _buildVehicleInfo(VehicleModel vehicle) {
    ...
    String? fullImageUrl;
    if(vehicle.photoPath != null) fullImageUrl = baseUrl + vehicle.photoPath!;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Transportasi', style: Get.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                ...
                child: Image.network(
                  ...
                  errorBuilder: (context, error, stackTrace) {
                    return Container(
                      ...
                      child: Center(child: Icon(Iconsax.car_outline, size: 50, color: Colors.grey.shade500)),
                    );
                  },
                ),
              ),
              const SizedBox(height: 10),
              Text(vehicle.name ?? 'Unknown Vehicle Name', style: Get.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                  ...
                  Flexible(
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(child: Text('Kapasitas: ${vehicle.capacity ?? 0} Orang', style: Get.textTheme.bodyMedium, maxLines: 1, overflow: TextOverflow.ellipsis)),
                      ],
                    ),
                  ),
                  ...
                  Flexible(
                    child: Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 5),
                        Flexible(child: Text('Tipe: ${vehicle.type != null ? vehicle.type!.capitalizeFirst : 'Unknown Vehicle Type'}', style: Get.textTheme.bodyMedium, maxLines: 1, overflow: TextOverflow.ellipsis)),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _buildAccommodationInfo(String? details) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Akomodasi', style: Get.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        _buildInfoCard(icon: Iconsax.home_trend_up_outline, title: 'Termasuk Penginapan', subtitle: details ?? 'Detail akomodasi tidak tersedia'),
        const SizedBox(height: 20),
      ],
    );
  }
  ...
  Widget _buildGuideInfo(String photoPath, String name, String specialization, String phone) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Pemandu Utama', style: Get.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        _buildInfoCard(
          ...
          trailing: IconButton(
            ...
            onPressed: () => controller.launchURL('tel:$phone'),
          ),
        ),
        const SizedBox(height: 20),
      ],
  ...
  Widget _buildGatheringPoint(String name, String url) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Titik Kumpul', style: Get.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        InkWell(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Tap untuk melihat di Google Maps', style: Get.textTheme.bodySmall?.copyWith(color: AppTheme.primaryColor)),
                    ],
                  ),
                ),
  ...
  Widget _buildReviewContent() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Reviews (${reviews.length})', style: Get.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
        const SizedBox(height: 15),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(name, style: Get.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                    Row(
    ...
    return Obx(() {
      ...
      return GestureDetector(
        ...
        child: Container(
          ...
          child: Text(tabName, style: Get.textTheme.bodyLarge?.copyWith(color: isSelected ? Colors.white : Colors.black54, fontWeight: FontWeight.bold),
          ),
        ),
      );
    });
                        ...
                        Expanded(
                          child: Row(
                            children: [
                              ...
                              Flexible(child: Text(location ?? '-', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16), maxLines: 1, overflow: TextOverflow.ellipsis)),
                            ],
                          ),
                        ),
                      ],
